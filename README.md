...
redirects.json                Cloudflare Bulk Redirect List source of truth
...
Production redirects are managed as code in `redirects.json`. On each deployment, the GitHub
Actions workflow validates the file and replaces all items in the Cloudflare Bulk Redirect
List with its contents. Cloudflare must have a Bulk Redirect Rule that enables the list.

```json
{
  "cloudflareList": "backyard_bullies_redirects",
  "allowEmpty": false,
  "redirects": [
    {
      "source": "backyardbullies-wc.com/join",
      "target": "https://www.backyardbullies-wc.com/#pricing",
      "statusCode": 301,
      "preserveQueryString": true,
      "comment": "Membership shortcut"
    }
  ]
}
```

The source may omit its scheme to match both HTTP and HTTPS. Supported optional settings are
`statusCode`, `preserveQueryString`, `includeSubdomains`, `subpathMatching`,
`preservePathSuffix`, and `comment`. Run `npm run lint:redirects` before committing.

The deployment requires these GitHub repository settings:

- Variable `CLOUDFLARE_ACCOUNT_ID`
- Secret `CLOUDFLARE_API_TOKEN`, scoped to **Account Filter Lists: Edit**

An empty file is not synchronized unless `allowEmpty` is explicitly changed to `true`. This
prevents an unfinished configuration from erasing the production redirect list.
